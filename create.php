...
<body class="theme-light">
  ...
  <?php if ($showToast): ?>
    <div class="site-toast" role="status" aria-live="polite">Haber paylaşıldı</div>
  <?php endif; ?>
    ...
    <section class="news-section">
      ...
      <form method="post" class="news-form">
        ...
        <label>
          ...
          <textarea id="contentEditor" name="content" rows="8"><?= htmlspecialchars($_POST['content'] ?? '') ?></textarea>
        ...
        <label>
          Görsel URL
          <input type="url" name="image" value="<?= htmlspecialchars($_POST['image'] ?? '') ?>" placeholder="https://...">
        </label>
        <label>
          Video URL
          <input type="url" name="video" value="<?= htmlspecialchars($_POST['video'] ?? '') ?>" placeholder="https://...">
        </label>
        <label>
          SEO URL
          <input type="text" name="slug" value="<?= htmlspecialchars($_POST['slug'] ?? '') ?>" placeholder="haber-baslik">
        ...
        <label class="checkbox-label">
          <input type="checkbox" name="featured" <?= isset($_POST['featured']) ? 'checked' : '' ?>> Öne çıkan haber yap
        </label>
        <label class="checkbox-label">
          <input type="checkbox" name="yz_help" <?= isset($_POST['yz_help']) ? 'checked' : '' ?>> YZ
        </label>
        <button type="submit" class="featured-action">Kaydet</button>
      </form>
    </section>
  </main>
  <script>
